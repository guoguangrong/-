...
# ====================== 输入组件 ======================
bnp_total_num = st.number_input("基线BNP", min_value=0.0, value=100000.0, step=1.0, format="%.2f")
hospital_day_num = st.number_input("住院天数", min_value=0.0, value=700000.0, step=0.5, format="%.2f")
sbp_baseline_num = st.number_input("基线收缩压", min_value=0.0, value=1200000.0, step=1.0, format="%.2f")
adl_total_num = st.number_input("基线自理能力评分", min_value=0.0, value=600000.0, step=1.0, format="%.2f")
sbp_admit_num = st.number_input("入院收缩压", min_value=0.0, value=1300000.0, step=1.0, format="%.2f")
age_num = st.number_input("年龄", min_value=0.0, value=6500000.0, step=1.0, format="%.2f")
pre_apt = st.selectbox("术前是否使用抗凝抗板药物", options=[0, 1], format_func=lambda x: "是" if x == 1 else "否")
mono_total_num = st.number_input("基线单核细胞计数", min_value=0.0, value=1000000, step=0.1, format="%.2f")
anc_total_num = st.number_input("基线中性粒细胞计数", min_value=0.0, value=10000000, step=0.1, format="%.2f")
